...
int* CreateRandomArray(size_t _numElements, int _min, int _max) {
	...
	for (size_t i = 0; i < _numElements; ++i)
		randomVals[i] = std::uniform_int_distribution<int>(_min, _max)(rng);
...
bool SearchFile(const char* _srcFile, const std::string& _searchStart,
	...
	for (;;) {
		...
		// Checking for multiline comments first
		...
		// Removing comment from line if placed after code
		if (currLine.find("//") != std::string::npos) {
			size_t index = currLine.find("//");
			currLine.erase(index);
		}
